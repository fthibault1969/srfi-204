  ...
  (syntax-rules ()
    ...
    ((match-one v (p q . r) g+s sk fk i)
     (match-check-ellipsis
      q
      (match-extract-prob p (match-gen-ellipsis v p r  g+s sk fk i) i ())
      (match-prob v (p q . r) g+s sk fk i)))
    ;; Go directly to MATCH-TWO.
    ((match-one . x)
     (match-prob . x))))

;; R6RS Helper functions
;; this may need to be changed, I've had trouble with cond-expand and 
;; R6RS

;; MATCH-PROB starts chain of checks for problem patterns,
;; otherwise passes on to MATCH-TWO.

;; MATCH-EXTRACT-PROB starts chain to extract problem patterns,
;; otherwise pass on to MATCH-EXTRACT-VARS.
...
;; MATCH-EXTRACT-PROB -> MATCH-EXTRACT-VARS chain
...
(define-syntax match-two
  (syntax-rules (___ *** quote quasiquote ? $ struct object = and or not set! get!)
    ...
    ((match-two v (or p ...) g+s sk fk i)
     (match-extract-prob (or p ...) (match-gen-or v (p ...) g+s sk fk i) i ()))
    ...
    ((match-two v (p) g+s sk fk i)
     (if (and (pair? v) (null? (cdr v)))
         ...
         fk))
    ((match-two v (p *** q) g+s sk fk i)
     (match-extract-prob p (match-gen-search v p q g+s sk fk i) i ()))
    ((match-two v (p *** . q) g+s sk fk i)
...
(define-syntax match-vector-tail
  (syntax-rules ()
    ((_ v p n len sk fk i)
     (match-extract-prob p (match-vector-tail-two v p n len sk fk i) i ()))))
...
(define-syntax match-extract-vars
  (syntax-rules (___ *** ? $ struct object = quote quasiquote and or not get! set!)
    ...
    ((match-extract-vars (p q . r) k i v)
     (match-check-ellipsis
      q
      (match-extract-prob (p . r) k i v)
      (match-extract-prob p (match-extract-vars-step (q . r) k i v) i ())))
    ((match-extract-vars (p . q) k i v)
     (match-extract-prob p (match-extract-vars-step q k i v) i ()))
...
(define-syntax match-extract-vars-step
  (syntax-rules ()
    ((_ p k i v ((v2 v2-ls) ...))
     (match-extract-prob p k (v2 ... . i) ((v2 v2-ls) ... . v)))
    ))
